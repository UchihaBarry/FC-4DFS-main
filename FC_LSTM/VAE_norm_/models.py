...
class SpiralAutoencoder(nn.Module):
    ...
    def forward(self, landmarks, neutral_points):
        landmarks=landmarks.reshape(landmarks.size()[0], landmarks.size()[1]*landmarks.size()[2])
        landmarks = landmarks.reshape(landmarks.size()[0], -1)
        
        X = self.decode(landmarks) # displacement -> landmarks(already done)
